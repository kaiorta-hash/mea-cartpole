...
class Brian2MEASimulator:
    ...
    def stimulate_neurons(self, pole_angle, pole_angular_velocity, reward):
        if np.abs(pole_angle) < 0.15:
            stim_wave = generate_stim_wave(pole_angle, pole_angular_velocity, 100)
        else:
            stim_wave = self.generate_random_noise(100)
            self.failure_count += 1

        normalized_stim = np.sqrt(np.mean(stim_wave**2)) * 0.05
        self.neurons.I = 0
        N_half = self.num_channels // 2
        target_neurons = range(N_half) if pole_angle < 0 else range(N_half, self.num_channels)
        for i in target_neurons:
            self.neurons.I[i] = normalized_stim + np.random.normal(0, 0.01)

        self.reward_trace = 0.9 * self.reward_trace + 0.1 * reward  # update reward trace (climbs twoard 1)

        for s in [self.synapses['left'], self.synapses['right']]:
            s.reward[:] = self.reward_trace  # broadcast reward trace  (changes synaptic weights didferently depending on STDP... look Apost or Apre logic)
